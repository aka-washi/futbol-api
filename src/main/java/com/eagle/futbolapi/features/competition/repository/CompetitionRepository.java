...
public interface CompetitionRepository extends JpaRepository<Competition, Long> {
        ...
        @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Competition c WHERE c.name = :name" +
                        " AND c.season = :season AND c.type = :type AND c.startDate = :startDate AND c.endDate = :endDate")
        ...
        @Query("SELECT CASE WHEN COUNT(c) > 0 THEN true ELSE false END FROM Competition c WHERE c.name = :name" +
                        " AND c.season = :season AND c.type = :type AND c.startDate = :startDate AND c.endDate = :endDate AND c.id <> :id")
